<script lang="ts">
  ...
  async function handleFormSubmit() {
    try {
        inputCipher = await invoke('handle_data', { outputSeed: outputSeed, password: password });
    } catch (error) {
...
<main>
  ...
  <h3>シードフレーズの復号</h3>
  <p>こちらのページでは、暗号化されたシードフレーズを復号することができます。暗号文とパスワードを入力して「復号する」ボタンをクリックしてください。</p>
